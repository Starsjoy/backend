...
export function usdtPremiumSlotKey(months) {
  return `usdt_premium_${months}`;
}
...
/**
 * POST /api/usdt-premium/order — Fragment orqali Premium (USDT), RobynHood emas.
 */
export async function createUsdtPremiumOrder(req, res, ctx) {
  const {
    pool,
    PREMIUM_3,
    PREMIUM_6,
  ...
  try {
    ...
    const slotKey = usdtPremiumSlotKey(monthsNum);
    ...
    const conflictPrices = await pool.query(
      `SELECT DISTINCT summ FROM orders 
       WHERE order_type IN ('gift', 'premium', 'premium_usdt', 'stars', 'stars_usdt') 
       AND status = 'pending' AND payment_status = 'pending'
       AND created_at >= NOW() - INTERVAL '8 minutes'`
    );
    ...
    console.log(
      `🧾 USDT Premium Order: #${order.id} | @${cleanUsername} | ${order.summ} so'm | ${monthsNum} oy`
    );
    ...
    console.error("❌ /api/usdt-premium/order error:", err);
